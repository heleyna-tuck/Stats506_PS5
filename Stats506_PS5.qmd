...
#Create serperate data frames for each variable:
...
Above we can see a plot with 4 variables being displayed. Both keys show what each color and lin type corresponds to. Using the graph above, we can tell that the Average PM10 variable has the least seasonal trend.
...
departure_delay_table <- flights_dt[, .(Mean_Departure_Delay = mean(dep_delay, na.rm = TRUE), Median_Departure_Delay = median(dep_delay, na.rm = TRUE)),by = .(origin)][order(-Mean_Departure_Delay)]
```

```{r}
# Generate a table reporting mean and median arrival delay per airport
arrival_delay_table <- flights_dt[, .(Mean_Arrival_Delay = mean(arr_delay, na.rm = TRUE), Median_Arrival_Delay = median(arr_delay, na.rm = TRUE)), by = .(dest)][order(-Mean_Arrival_Delay)]
```

```{r}
# Exclude destinations in the arrival table with under 10 flights
arrival_delay_table <- arrival_delay_table[ , .(Mean_Arrival_Delay, Median_Arrival_Delay), by = .(dest)][flights_dt[, .N, by = .(dest)][N >= 10]$dest]
```
...
flights_dt[, air_time := air_time / 60]  # Convert air_time to hours
flights_dt[, average_speed := distance / air_time]

# Find the aircraft model with the fastest average speed
fastest_model <- flights_dt[, .(Average_Speed_MPH = max(average_speed, na.rm = TRUE), Number_of_Flights = .N), by = .(tailnum = model)][order(-Average_Speed_MPH)][1]
